...
import com.intellij.codeInsight.completion.{CompletionPhase, CompletionService}
...
private object ScalaPsiEventFilter {
  ...
  private object IgnoreLeafElementsFilter extends ScalaPsiEventFilter {
    ...
    override def shouldSkip(element: PsiElement): Boolean = element.getNode.getElementType match {
      case ERROR_ELEMENT | BAD_CHARACTER | WHITE_SPACE => true
      case scalaToken: ScalaTokenType                  => !significantTokens.contains(scalaToken)
      case _                                           => super.shouldSkip(element)
    }
  }
}
